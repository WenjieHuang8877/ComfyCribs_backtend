...
public class StayController {
    ...
    // Constructor with dependency injection for StayService and ReservationService.
    public StayController(StayService stayService, ReservationService reservationService) {
        this.stayService = stayService;
        this.reservationService = reservationService;
    }

    // Returns a list of Stays for the currently authenticated user.
    @GetMapping(value = "/stays")
    public List<Stay> listStays(Principal principal) {
        return stayService.listByUser(principal.getName());
    }

    // Returns a specific Stay object identified by stayId for the currently authenticated user.
    ...
    // Adds a new Stay to the database with the provided parameters.
    @PostMapping("/stays")
    public void addStay(
    ...
    // Deletes a Stay from the database identified by stayId.
    @DeleteMapping("/stays/{stayId}")
    public void deleteStay(@PathVariable Long stayId, Principal principal) {
        stayService.delete(stayId, principal.getName());
    }

    // Returns a list of Reservations for a specific Stay identified by stayId.
